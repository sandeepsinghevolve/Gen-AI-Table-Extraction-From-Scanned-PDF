...
def get_table_download_link(df):
    ...
    excel_buffer.seek(0)
    excel_binary = excel_buffer.read()
    excel_base64 = base64.b64encode(excel_binary).decode()
    href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{excel_base64}" download="extracted_table.xlsx">Download Table as Excel</a>'
    return href
...
if submit:
    ...
    rows = [row.replace("|","").split("\t") for row in extracted_table_output.split("\n")]
